...
// operationType: 操作类型
// resourceID: 资源ID
// updateRequiresNoRef: update操作是否需要引用计数为0
// 返回：是否可以执行，错误信息
func (m *RefCountManager) CanPerformOperation(operationType, resourceID string, updateRequiresNoRef bool) (bool, string) {
	refCount := m.GetRefCount(resourceID)

	switch operationType {
	case OperationTypePull:
		// Pull操作：如果refcount != 0，应该跳过，但不阻止
		// 这里返回true，由ShouldSkipOperation判断是否跳过
		return true, ""

	case OperationTypeDelete:
		// Delete操作：如果refcount > 0，不能执行
		if refCount.Count > 0 {
			return false, "无法删除：当前有节点正在使用该资源"
		}
		return true, ""
